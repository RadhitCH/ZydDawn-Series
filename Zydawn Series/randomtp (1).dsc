...
random_teleport_on_spawn:
    ...
    events:
        ...
        after player login:
            ...
            - else:
                - flag <player> join
                - define x <util.random.int[-<script[config_zydawn].parsed_key[rtp location]>].to[<script[config_zydawn].parsed_key[rtp location]>]>
                - define y <util.random.int[130].to[200]>
                - define z <util.random.int[-<script[config_zydawn].parsed_key[rtp location]>].to[<script[config_zydawn].parsed_key[rtp location]>]>
                - cast DAMAGE_RESISTANCE duration:20s amplifier:100 hide_particles no_icon no_ambient
                - teleport <player> <[x]>,<[y]>,<[z]>,<world[<script[config_zydawn].parsed_key[rtp world name]>]>    
